function handles= plot_results(parameter,truth,meas,est)
...
for i=1:truth.total_tracks
    Zt= gen_observation_fn( parameter, X_track(:,k_birth(i):1:k_death(i),i));
    polar( -Zt(1,:)+pi/2, Zt(2,:),'k-'  );
    polar( -Zt(1,1)+pi/2, Zt(2,1), 'ko');
    polar( -Zt(1,k_death(i)-k_birth(i)+1)+pi/2, Zt(2,k_death(i)-k_birth(i)+1),'k^');
end
axis equal; axis([-parameter.range_c(2,2) parameter.range_c(2,2) 0 parameter.range_c(2,2)]); title('Ground Truths');
...
set(gca, 'XLim',[1 truth.K]); set(gca, 'YLim',[-parameter.range_c(2,2) parameter.range_c(2,2)]);
legend([hline2 hline1 hlined],'Estimates','True tracks','Measurements');

subplot(212); xlabel('Time'); ylabel('y-coordinate (m)');
set(gca, 'XLim',[1 truth.K]); set(gca, 'YLim',[ parameter.range_c(1,2) parameter.range_c(2,2)] );
%legend([yhline2 yhline1 yhlined],'Estimates          ','True tracks','Measurements');



%return
handles=[ truths tracking];
